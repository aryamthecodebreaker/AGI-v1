...
export async function peopleRoutes(app: FastifyInstance, storage: Storage): Promise<void> {
  ...
  app.get('/api/people', { preHandler: auth }, async (req) => {
    const user = req.user!;
    const people = await storage.people.listByUser(user.id);
    return people.map((p) => ({
      id: p.id,
  ...
  app.get<{ Params: { id: string } }>('/api/people/:id', { preHandler: auth }, async (req, reply) => {
    const user = req.user!;
    const person = await storage.people.getById(req.params.id);
    if (!person || person.userId !== user.id) {
      return reply.status(404).send({ error: 'person not found' });
    }
    const memories = (await storage.personMemories.getMemoriesForPerson(person.id, 50))
