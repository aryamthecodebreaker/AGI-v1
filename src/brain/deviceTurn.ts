...
export interface DeviceTurnInput {
  ...
  storage: Storage;
  /**
   * Resolved lazily, and only when the planner actually runs. Device queries,
   * corrections, retries and confirmations are answered deterministically from
   * stored state, so they must not require an LLM to be configured at all.
   */
  llm: () => LlmBackend;
  userId: string;
...
export function countOnlineDevices(storage: Storage, userId: string, offlineAfterMs: number): number {
